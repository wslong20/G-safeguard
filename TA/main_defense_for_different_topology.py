...
async def no_defense_communication(ag: AgentGraph, case, num_dialogue_turns): 
    communication_data = []
...
async def defense_communication(ag:AgentGraphWithDefense, gnn: MyGAT, case, adj_m: np.ndarray,  num_dialogue_turns): 
    communication_data = []
    response_embeddings = []
    initial_responses = await ag.afirst_generate(case)
...
def parse_arguments():
    ...
    parser.add_argument("--dataset_path", type=str, default="./agent_graph_dataset/tool_attack\dataset.json", help="Save path of the dataset")
    parser.add_argument("--graph_type", type=str, choices=["random", "chain", "tree", "star"], default="random")
    parser.add_argument("--gnn_checkpoint_path", type=str, default=r".\checkpoint\tool_attack\checkpoint.pth")
...
async def main(): 
    ...
    for d in tqdm(dataset): 
        ...
        try:
            ...
            communication_data_no_defense = await no_defense_communication(agnd, case, num_dialogue_turns)
            communication_data_defense = await defense_communication(agwd, gnn, case, adj_m, num_dialogue_turns)
