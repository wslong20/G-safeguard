...
def generate_undirected_graph_dataset(num_nodes, sparsity, num_graphs):
    if not (0 <= sparsity <= 1):
        raise ValueError("Sparsity must be a value between 0 and 1.")
    ...
    for _ in range(num_graphs):
        upper_tri = np.triu(np.random.rand(num_nodes, num_nodes) <= sparsity, 1)
...
def generate_agent_graph_dataset(num_nodes, sparsity, num_graphs, num_attackers): 
    ...
    dataset = []
    adj_matrices = generate_undirected_graph_dataset(num_nodes, sparsity, num_graphs)
...
async def generate_graph_dataset(args): 
    ...
    for d in tqdm(sampled_initial_dataset, desc="Generate communication data"): 
        try: 
            ...
            communication_data.append(copy.copy(ag.communication_info_matrix.tolist()))
